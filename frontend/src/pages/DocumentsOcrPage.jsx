...
function latest(items, attachment) {
  return items.filter((item) => item.attachment === attachment).sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0];
}
...
function makeReview(analysis) {
  ...
  return {
    ...
    items: (proposal.items || []).map((item, index) => ({
      ...
      variant_id: saved.items?.[index]?.variant_id ?? "",
      description: saved.items?.[index]?.description ?? item.description ?? "",
...
export default function DocumentsOcrPage() {
  ...
  const load = async () => { try { const [a, b, c, d, e] = await Promise.all([request("/documents/documents/"), request("/documents/types/?is_active=True"), request("/documents/attachments/"), request("/documents/ocr-analyses/"), request("/catalog/variants/?page_size=200")]); setDocuments(list(a)); setTypes(list(b)); setAttachments(list(c)); setAnalyses(list(d)); setVariants(list(e)); } catch (error) { notice(error.message, false); } };
  useEffect(() => { load(); }, []);
  ...
  const closeReview = () => { setReviewAnalysis(null); setReview(null); };
  ...
  return <section className="products-page documents-page">
    ...
    <article className="products-list"><div className="document-heading"><span>Documento</span><span>Tipo</span><span>Data</span><span>PDF e OCR</span><span>Stato</span><span>Azioni</span></div>{!visible.length ? <div className="empty-product-state"><FileText size={28} /><strong>Nessun documento trovato</strong><span>Carica un PDF per costruire l'archivio documentale.</span></div> : visible.map((document) => { const attachment = attachmentFor(document.id); const analysis = attachment && latest(analyses, attachment.id); const reviewed = analysis?.proposed_data?.review?.status === "CONFIRMED"; return <div className="document-row" key={document.id}><div><strong>{document.title}</strong><span>{document.number || "Numero non indicato"}{document.counterparty_name ? ` · ${document.counterparty_name}` : ""}</span></div><span>{typeFor(document.document_type)?.name || directionLabel[document.direction]}</span><span>{date(document.document_date)}</span><span>{analysis?.status === "SUCCEEDED" ? reviewed ? "Proposta confermata" : "OCR completato" : analysis?.status === "FAILED" ? "OCR non disponibile" : attachment ? "PDF pronto" : "Nessun PDF"}</span><em className={`document-status ${document.status.toLowerCase()}`}>{statusLabel[document.status] || document.status}</em><div className="document-actions">{analysis?.status === "SUCCEEDED" && <button title="Apri proposta OCR" onClick={() => { setReviewAnalysis(analysis); setReview(makeReview(analysis)); }}><Eye size={16} /></button>}{attachment && <button title="Analizza fattura con OCR" onClick={() => analyze(attachment)}><ScanText size={16} /></button>}{document.status === "DRAFT" && reviewed && <button title="Registra documento" onClick={() => finalize(document)}><CheckCircle2 size={16} /></button>}{attachment?.file && <a href={attachment.file} target="_blank" rel="noreferrer" title="Apri PDF"><FileUp size={16} /></a>}</div></div>; })}</article>
    {reviewAnalysis && review && <div className="ocr-review-layer" role="dialog" aria-modal="true"><button className="ocr-review-backdrop" aria-label="Chiudi" onClick={closeReview} /><section className="ocr-review-modal"><header><div><p className="eyebrow">Proposta OCR</p><h3>Controlla gli articoli della fattura</h3><span>Includi solo le righe corrette e, se vuoi, collegale a un articolo del catalogo.</span></div><button type="button" title="Chiudi" onClick={closeReview}><X size={20} /></button></header><div className="ocr-review-summary"><label>Fornitore<input value={review.supplier_name} onChange={(event) => updateReview("supplier_name", event.target.value)} /></label><label>Numero fattura<input value={review.invoice_number} onChange={(event) => updateReview("invoice_number", event.target.value)} /></label><label>Data<input type="date" value={review.invoice_date} onChange={(event) => updateReview("invoice_date", event.target.value)} /></label><label>Imponibile articoli<input readOnly value={money(reviewSubtotal)} /></label><label>IVA<input inputMode="decimal" value={review.tax_amount} onChange={(event) => updateReview("tax_amount", event.target.value)} /></label><label>Totale fattura<input readOnly value={money(reviewTotal)} /></label></div><div className="ocr-review-lines"><div className="ocr-review-heading"><span>Incl.</span><span>Descrizione</span><span>Articolo in catalogo</span><span>Qta</span><span>Prezzo</span><span>Totale</span></div>{review.items.length ? review.items.map((item, index) => <div className={`ocr-review-line${item.accepted ? "" : " is-discarded"}`} key={index}><label><input type="checkbox" checked={item.accepted} onChange={(event) => updateItem(index, "accepted", event.target.checked)} /><span className="sr-only">Includi articolo</span></label><input value={item.description} onChange={(event) => updateItem(index, "description", event.target.value)} /><select value={item.variant_id} onChange={(event) => updateItem(index, "variant_id", event.target.value)}><option value="">Non associare ora</option>{variants.map((variant) => <option value={variant.id} key={variant.id}>{variant.product_name || "Articolo"} · {variant.sku}</option>)}</select><input inputMode="numeric" value={item.quantity} onChange={(event) => updateItem(index, "quantity", event.target.value)} /><input inputMode="decimal" value={item.unit_price} onChange={(event) => updateItem(index, "unit_price", event.target.value)} /><input readOnly value={money(lineTotal(item))} /></div>) : <p className="ocr-review-empty">L’OCR non ha riconosciuto righe articolo in questo PDF.</p>}</div><footer><p>Confermare salva questa proposta, ma non aggiorna automaticamente il magazzino.</p><span><button type="button" className="secondary-action" onClick={closeReview}>Scarta modifiche</button><button className="primary-action" disabled={reviewSaving} onClick={saveReview}>{reviewSaving ? "Salvataggio..." : "Conferma proposta"}</button></span></footer></section></div>}
  </section>;
}
