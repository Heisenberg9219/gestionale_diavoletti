...
export default function DocumentsOcrPage() {
  ...
  useEffect(() => {
    if (!reviewError && !reviewNotice) return;
    const timer = setTimeout(() => { setReviewError(""); setReviewNotice(""); }, 4000);
  ...
  async function saveReview() {
    ...
    try {
      ...
      if (validation.conflicts.length) {
        setReviewError(validation.conflicts.join("\n"));
        return;
      }
